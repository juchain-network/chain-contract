...
func showUsageGuide(cmd *cobra.Command, _ []string) {
	...
	fmt.Println("  5: blockReward (block reward)")
	fmt.Println("  6: unbondingPeriod (unbonding period)")
	fmt.Println("  7: validatorUnjailPeriod (validator unjail period)")
	fmt.Println("  8: minValidatorStake (minimum validator stake)")
	fmt.Println("  9: maxValidators (maximum validators)")
	fmt.Println(" 10: minDelegation (minimum delegation)")
	fmt.Println(" 11: minUndelegation (minimum undelegation)")
	fmt.Println(" 12: doubleSignSlashAmount (double-sign slash amount)")
	...
	fmt.Println("  Note: Additional validators can be added via governance proposals")
	fmt.Println()

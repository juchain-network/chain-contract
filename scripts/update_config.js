...
async function main() {
    ...
    // 获取原值
    let ov = await proposal.proposalLastingPeriod();
    console.log('proposalLastingPeriod', ov.toString());
    ...
    // 5: increasePeriod
    // 6: receiverAddr
    let newValue = 8 * 24 * 3600;
    let tx = await proposal.connect(miner1).createUpdateConfigProposal(0, newValue);
    let receipt = await tx.wait();
    console.log("miner1 create proposal tx:", tx.hash);
    let ev = receipt.events.find(event => event.event == "LogCreateConfigProposal");
    const [id, proposer, cid, nv, t] = ev.args;
    console.log("proposal-id:", id, "proposer:", proposer, "cid:", cid, "new value", nv, "time:", t)
    ...
    let newV = await proposal.proposalLastingPeriod();
    console.log('proposalLastingPeriod', newV.toString());
}

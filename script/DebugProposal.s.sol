...
contract DebugProposalScript is BaseSetup {
    ...
    function testCreateProposal() external {
        ...
        vm.prank(initVals[0]);
        try Proposal(PROPOSAL).createProposal(testTarget, true, details) returns (bytes32 proposalId) {
            console.log("Create proposal result: Proposal ID =", vm.toString(proposalId));
        } catch Error(string memory reason) {

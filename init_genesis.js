...
function generateStakingStorage() {
  ...
  function getArraySlot(slot, index) {
    const baseSlot = keccak256(
      Buffer.from(slot.toString(16).padStart(64, "0"), "hex"),
    );
  ...
  INITIAL_VALIDATORS.forEach((validator, index) => {
    ...
    // selfStake (offset 0)
    storage[stakingSlot] =
      "0x" + minValidatorStake.toString(16).padStart(64, "0");

    // totalDelegated (offset 1)
    const totalDelegatedSlot =
      "0x" + (BigInt(stakingSlot) + BigInt(1)).toString(16).padStart(64, "0");
    storage[totalDelegatedSlot] = "0x" + "0".padStart(64, "0");

    // commissionRate (offset 2) - 5% = 500 basis points
    const commissionSlot =
      "0x" + (BigInt(stakingSlot) + BigInt(2)).toString(16).padStart(64, "0");
    storage[commissionSlot] = "0x" + "500".padStart(64, "0");

    // accumulatedRewards (offset 3)
    const rewardsSlot =
      "0x" + (BigInt(stakingSlot) + BigInt(3)).toString(16).padStart(64, "0");
    storage[rewardsSlot] = "0x" + "0".padStart(64, "0");

    // isJailed (offset 4) - false = 0
    const jailSlot =
      "0x" + (BigInt(stakingSlot) + BigInt(4)).toString(16).padStart(64, "0");
    storage[jailSlot] = "0x" + "0".padStart(64, "0");

    // jailUntilBlock (offset 5) - 0
    const jailUntilSlot =
      "0x" + (BigInt(stakingSlot) + BigInt(5)).toString(16).padStart(64, "0");

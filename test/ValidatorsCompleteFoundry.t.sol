...
contract RejectingFeeReceiver {
    function withdrawProfits(Validators validators, address validator) external {
...
contract ValidatorsCompleteFoundryTest is BaseSetup {
    ...
    function testCreateValidatorSuccess() public {
        ...
        require(
            Validators(VALIDATORS).isTopValidator(validator),
            "validator should be in highestValidatorsSet after registration"
        );
    ...
    function testExistingValidatorCanEditInfoAfterPassCleared() public {
        address feeAddr = makeAddr("feeAddrAfterRemoval");

        _passProposal(v1, false);

        require(!Proposal(PROPOSAL).pass(v1), "v1 should be unpassed");
        require(Validators(VALIDATORS).isValidatorExist(v1), "v1 should remain a registered validator");

        vm.prank(v1);
        bool success = Validators(VALIDATORS)
            .createOrEditValidator(
                payable(feeAddr), "edited-validator", "identity", "https://example.com", "ops@example.com", "details"
            );
        require(success, "existing validator should still be able to edit info");

        (address payable actualFeeAddr,,,,) = Validators(VALIDATORS).getValidatorInfo(v1);
        require(actualFeeAddr == feeAddr, "fee address should update after pass is cleared");

        (string memory moniker,,,,) = Validators(VALIDATORS).getValidatorDescription(v1);
        require(keccak256(bytes(moniker)) == keccak256(bytes("edited-validator")), "metadata should update");
    }
    ...
    function testValidatorCanRotateFeeAddrAfterResignAndWithdrawProfits() public {
        RejectingFeeReceiver rejectingFeeReceiver = new RejectingFeeReceiver();
        address newFeeAddr = makeAddr("newFeeAddr");
        uint256 fee = 0.3 ether;

        vm.prank(v1);
        Validators(VALIDATORS).createOrEditValidator(payable(address(rejectingFeeReceiver)), "", "", "", "", "");
        ...
        require(!Proposal(PROPOSAL).pass(v1), "v1 should be unpassed after resign");

        vm.expectRevert("Transfer failed");
        rejectingFeeReceiver.withdrawProfits(Validators(VALIDATORS), v1);

        vm.prank(v1);
        Validators(VALIDATORS).createOrEditValidator(payable(newFeeAddr), "", "", "", "", "");

        vm.expectRevert("You are not the fee receiver of this validator");
        rejectingFeeReceiver.withdrawProfits(Validators(VALIDATORS), v1);

        uint256 feeBalBefore = newFeeAddr.balance;
        vm.prank(newFeeAddr);
        Validators(VALIDATORS).withdrawProfits(v1);
        uint256 feeBalAfter = newFeeAddr.balance;

        require(feeBalAfter - feeBalBefore == fee, "new fee address should receive preserved profits");
    }
    ...
    function testCantWithdrawWithoutProfits() public {
        ...
        vm.prank(feeAddr);
        (bool ok,) =
            address(Validators(VALIDATORS)).call(abi.encodeWithSelector(Validators.withdrawProfits.selector, miner));
    ...
    function testRemoveValidatorIncoming() public {
        ...
        require(
            v2Incoming >= expectedReward && v3Incoming >= expectedReward,
            "Rewards should be distributed to remaining validators"
        );
    }
    ...
    function testValidatorSetCompetition() public {
        ...
        for (uint256 i = 0; i < highestSet.length; i++) {

...
contract StakingAdditionalCoverageTest is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(deployer);
        punish.initialize(VALIDATORS, PROPOSAL, STAKING);
        vm.stopPrank();

        // Set up validators for testing
        vm.startPrank(deployer);
        // Add test validator to validators list
        vm.mockCall(VALIDATORS, abi.encodeWithSelector(Validators.isActiveValidator.selector), abi.encode(true));

        // Mock tryActive to always succeed
        vm.mockCall(VALIDATORS, abi.encodeWithSelector(Validators.tryActive.selector), abi.encode(true));

        // Mock cleanPunishRecord to always succeed
        vm.mockCall(PUNISH, abi.encodeWithSelector(Punish.cleanPunishRecord.selector), abi.encode(true));
        vm.stopPrank();
    }
    ...
    function testExitValidatorWithRemoveFromHighestSet() public {
        ...
        vm.mockCall(VALIDATORS, abi.encodeWithSelector(Validators.isActiveValidator.selector), abi.encode(false));
        ...
        vm.mockCall(
            VALIDATORS, abi.encodeWithSelector(Validators.removeFromHighestSet.selector, VALIDATOR1), abi.encode()
        );

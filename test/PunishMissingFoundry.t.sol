...
contract PunishMissingFoundryTest is BaseSetup {
    ...
    function testPunishRecordCleaning() public {
        ...
        vm.coinbase(VALIDATORS);
        vm.startPrank(VALIDATORS);
        // Total 48 times to reach removal threshold
    ...
    function testExecutePendingLimitZero() public {
        ...
        vm.prank(miner);
        punish.executePending(0);
    }

    function testExecutePendingLimitTooLarge() public {
        Punish punish = Punish(PUNISH);
        address miner = makeAddr("miner");
        vm.coinbase(miner);
        ...
        vm.prank(miner);
        punish.executePending(6);
    }

    function testExecutePendingOnlyMiner() public {
        address miner = makeAddr("miner");
        vm.coinbase(miner);

        vm.expectRevert("Miner only");
        Punish(PUNISH).executePending(1);
    }

...
contract StakingTest is Test {
    ...
    function test_RevertWhen_AddStakeToJailedValidator() public {
        ...
        (uint256 selfStake,,,,,,,,,) = Staking(STAKING).getValidatorInfo(VALIDATOR1);
        assertEq(
            selfStake, MIN_STAKE + additionalStake, "Stake should be successfully added even when validator is jailed"
        );
    }
    ...
    function testWithdrawUnbondedMultipleEntries() public {
        ...
        for (uint256 i = 0; i < undelegateAmounts.length; i++) {
    ...
    function testUpdateCommissionRate_RevertWhen_Jailed() public {
        ...
        assertEq(
            updatedCommissionRate, COMMISSION_RATE, "Commission rate should be updated successfully even when jailed"
        );
    }
    ...
    function testClaimRewardsQueuesPendingPayoutWhenReceiverRejects() public {
        _setupValidatorPass(VALIDATOR1);
        vm.prank(VALIDATOR1);
        Staking(STAKING).registerValidator{value: MIN_STAKE}(COMMISSION_RATE);

        RejectingReceiver receiver = new RejectingReceiver();
        vm.deal(address(receiver), MIN_DELEGATION);
        receiver.delegateTo{value: MIN_DELEGATION}(STAKING, VALIDATOR1);
        ...
        receiver.claimRewardsFrom(STAKING, VALIDATOR1);

        uint256 queued = Staking(STAKING).pendingPayouts(address(receiver));
        assertGt(queued, 0);

        uint256 balanceBefore = DELEGATOR1.balance;
        receiver.withdrawPendingPayoutTo(STAKING, payable(DELEGATOR1));
        assertEq(Staking(STAKING).pendingPayouts(address(receiver)), 0);
        assertEq(DELEGATOR1.balance, balanceBefore + queued);
    }
    ...
    function testSlashQueuesPendingPayoutWhenReceiverRejects() public {
        ...
        vm.prank(PUNISH);
        (uint256 actualSlash, uint256 actualReward) =
            Staking(STAKING).slashValidator(VALIDATOR1, slashAmount, address(receiver), rewardAmount, burnAddress);

        assertEq(actualSlash, slashAmount);
        assertEq(actualReward, rewardAmount);
        assertEq(Staking(STAKING).pendingPayouts(address(receiver)), rewardAmount);
    }
...
contract RejectingReceiver {
    receive() external payable {
        revert("reject receive");
    }

    function delegateTo(address staking, address validator) external payable {
        Staking(staking).delegate{value: msg.value}(validator);
    }

    function claimRewardsFrom(address staking, address validator) external {
        Staking(staking).claimRewards(validator);
    }

    function withdrawPendingPayoutTo(address staking, address payable recipient) external {
        Staking(staking).withdrawPendingPayout(recipient);
    }
}

...
# - 验证者7将同时在Validators和Staking合约中活跃
#
# 依赖要求：
...
PASSWORDS=(
    ...
    "${PRIVATE_CHAIN_PATH}/data-validator6/password.txt"
)
...
echo "查询验证者7状态 (Query Validator 7 Status)..."
...
echo "🎉 流程完成！验证者7已经成功添加到网络中。"
echo "🎉 Process completed! Validator 7 has been successfully added to the network."
echo ""
...
async function transferFunds() {
    ...
    const toAddress = '0x50c554ac9c134491818fa6f21d504f2ae5bd9c26';
    const amount = web3.utils.toWei('10010', 'ether');
    
    console.log('🚀 转账10010 ETH给验证者7...');
    ...
    try {
        ...
        console.log('验证者7新余额:', web3.utils.fromWei(balance, 'ether'), 'ETH');
...
./build/congress-cli staking edit-validator \
  ...
  --moniker "validator7" \
  --details "Validator7 node with fee address configured" || {
    echo "ℹ️  如果编辑验证者命令失败，验证者仍然可以正常工作"
    echo "ℹ️  收费地址设置可以稍后通过其他方式配置"
}

# 查找验证者7的密钥文件
VALIDATOR7_KEY=$(find "${PRIVATE_CHAIN_PATH}/data-validator7/keystore/" -name "*--50c554ac9c134491818fa6f21d504f2ae5bd9c26" | head -1)
VALIDATOR7_PASSWORD="${PRIVATE_CHAIN_PATH}/data-validator7/password.txt"

if [ -f "editValidator.json" ] && [ -n "$VALIDATOR7_KEY" ] && [ -f "$VALIDATOR7_PASSWORD" ]; then
    echo "签名编辑验证者交易..."
    ./build/congress-cli sign -f editValidator.json -k "$VALIDATOR7_KEY" -p "$VALIDATOR7_PASSWORD"
...
# 4.3 注册到Staking合约并质押
echo "🚀 步骤4.3: 注册验证者7到Staking合约并质押..."
...
if [ -f "registerValidator.json" ] && [ -n "$VALIDATOR7_KEY" ] && [ -f "$VALIDATOR7_PASSWORD" ]; then
    echo "签名Staking注册交易..."
    ./build/congress-cli sign -f registerValidator.json -k "$VALIDATOR7_KEY" -p "$VALIDATOR7_PASSWORD"
...
echo "📋 完整的miners列表（应显示7个验证者）："
./build/congress-cli miners
echo ""

echo "🎉 验证者7完整配置流程完成！"
echo "🎉 Validator 7 complete configuration process completed!"
echo ""
echo "📊 网络现在有7个活跃验证者 (Network now has 7 active validators)："
...
echo "   - Validator6: 0x9965507d1a55bcc2695c58ba16fb37d819b0a4dc"
echo "   - Validator7: 0x50c554ac9c134491818fa6f21d504f2ae5bd9c26 ✨ (新添加 Newly Added)"
echo ""
echo "✨ 完整功能："
echo "   - ✅ 自动创建提案并投票通过（现在使用6个验证者投票）"
echo "   - ✅ 自动为验证者7转账10010 ETH"
echo "   - ✅ 自动设置收费地址"
echo "   - ✅ 自动注册到Staking合约并质押10000 JU"
echo "   - ✅ 验证者7现在同时在Validators和Staking合约中活跃"
...
echo "ℹ️  此脚本基于6个验证者的投票机制，确保达到所需的多数票"
echo "ℹ️  This script uses a 6-validator voting mechanism to ensure the required majority"

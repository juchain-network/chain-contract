...
contract RewardFoundryTest is BaseSetup {
    ...
    function testRewardEquallyDistributedNoStake() public {
        ...
        vm.prank(miner);
        Validators(VAL).distributeBlockReward{value: reward}();
    ...
    function testRemoveValidatorReward() public {
        ...
        uint256 threshold = Proposal(PRO).punishThreshold();
        
        // 先给 v1 一些奖励
        vm.prank(miner);
        Validators(VAL).distributeBlockReward{value: 1 ether}();
        
        (, , uint256 toRemoveBefore,,) = Validators(VAL).getValidatorInfo(v1);
        (, , uint256 v2Before,,) = Validators(VAL).getValidatorInfo(v2);
        (, , uint256 v3Before,,) = Validators(VAL).getValidatorInfo(v3);
        
        // 惩罚 v1 达到阈值
        for (uint i = 0; i < threshold; i++) {
            vm.coinbase(miner);
            vm.prank(miner);
            Punish(PUN).punish(v1);
            vm.roll(block.number + 1);
        }
        
        // v1 的奖励应该被移除并分配给其他验证者
        (, , uint256 v1After,,) = Validators(VAL).getValidatorInfo(v1);
        (, , uint256 v2After,,) = Validators(VAL).getValidatorInfo(v2);
        (, , uint256 v3After,,) = Validators(VAL).getValidatorInfo(v3);
    ...
    function testJailedValidatorCantGetReward() public {
        ...
        uint256 removeThreshold = Proposal(PRO).removeThreshold();
    ...
    function testJailedValidatorCantGetPunishProfits() public {
        ...
        (, , uint256 v1Before,,) = Validators(VAL).getValidatorInfo(v1);
        (, , uint256 v2Before,,) = Validators(VAL).getValidatorInfo(v2);
        (, , uint256 v3Before,,) = Validators(VAL).getValidatorInfo(v3);
